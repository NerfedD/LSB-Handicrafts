...
import UpdateCredentials from './components/views/UpdateCredentials';
import CreateUserAccount from './components/views/CreateUserAccount';
...
function App() {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      case 'login':
        // We pass setCurrentScreen down as "onNavigate"
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* Renders the actual screen here */}
      {renderScreen()}

      {/* --- TEMPORARY DEVELOPER NAVIGATION BAR --- */}
      <div className="fixed bottom-0 left-0 w-full bg-slate-900 text-white p-3 flex justify-center gap-2 overflow-x-auto z-[9999] shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        <span className="text-xs text-slate-400 font-bold uppercase tracking-widest mr-4 flex items-center">
          Dev Menu
        </span>
        <button 
          onClick={() => setCurrentScreen('login')}
          className={`px-3 py-1.5 text-xs rounded transition-colors ${currentScreen === 'login' ? 'bg-blue-600' : 'bg-slate-800 hover:bg-slate-700'}`}
        >
          1. Login
        </button>
        <button 
          onClick={() => setCurrentScreen('forgot')}
          className={`px-3 py-1.5 text-xs rounded transition-colors ${currentScreen === 'forgot' ? 'bg-blue-600' : 'bg-slate-800 hover:bg-slate-700'}`}
        >
          2. Forgot Pass
        </button>
        <button 
          onClick={() => setCurrentScreen('update')}
          className={`px-3 py-1.5 text-xs rounded transition-colors ${currentScreen === 'update' ? 'bg-blue-600' : 'bg-slate-800 hover:bg-slate-700'}`}
        >
          3. Update Creds
        </button>
        <button 
          onClick={() => setCurrentScreen('create')}
          className={`px-3 py-1.5 text-xs rounded transition-colors ${currentScreen === 'create' ? 'bg-blue-600' : 'bg-slate-800 hover:bg-slate-700'}`}
        >
          4. Create User
        </button>
        <button 
          onClick={() => setCurrentScreen('users')}
          className={`px-3 py-1.5 text-xs rounded transition-colors ${currentScreen === 'users' ? 'bg-blue-600' : 'bg-slate-800 hover:bg-slate-700'}`}
        >
          5/6. User Accounts
        </button>
      </div>
