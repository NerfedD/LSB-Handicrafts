...
/** Only sign-in needs a client notification; the server derives and deduplicates it. */
export async function record({ kind }) {
  // Business events are now generated by database triggers in the transaction.
  // Keep this call boundary for existing callers, but never accept their prose,
  // actor or timestamp as an authoritative event.
  if (kind !== ACTIVITY_KIND.SIGN_IN) return;
  try {
