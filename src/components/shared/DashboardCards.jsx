  ...
  return (
    <div className="flex flex-col items-center justify-center px-8 py-14 text-center">
      <div className="flex size-11 items-center justify-center rounded-full bg-[#17263a0a]">
        <Activity className="h-[18px] w-[18px] text-[#5f6875]/60" />
      </div>
      <p className="pt-4 text-[14px] font-bold text-[#17263a]">{title}</p>
      <p className="max-w-[340px] pt-1.5 text-[12.5px] leading-[1.55] text-[#5f6875]">
        {description}
      </p>
    </div>
  );
...
export function QuickAction({ icon, label, onClick, primary = false }) {
  return (
    <button
      ...
      className={
        primary
          ? "flex h-[42px] items-center gap-2 rounded-[9px] bg-[#1746d1] px-[18px] text-[13.5px] font-semibold text-white shadow-[0_2px_6px_rgba(23,70,209,0.24)] transition hover:bg-[#1238ad]"
          : "flex h-[42px] items-center gap-2 rounded-[9px] border border-[#17263a2e] px-[18px] text-[13.5px] font-semibold text-[#17263a] transition hover:bg-[#17263a08]"
      }
    >
      {icon}
      {label}
