  ...
  return (
    <div className="animate-in fade-in duration-300 w-full">
      <div className="bg-white dark:bg-[#111116] border border-zinc-200 dark:border-[#1F1F2E] rounded-2xl overflow-hidden shadow-sm dark:shadow-lg w-full">
        ...
        {/* //ALLU: Keep desktop table layout for medium+ screens. */}
        ...
        {/* //ALLU: Add mobile cards to prevent squeezed columns and action misalignment on narrow screens. */}
        <div className="md:hidden p-4 space-y-3">
          {filteredOrders.length === 0 ? (
            ...
            filteredOrders.map((order) => (
              <div key={order.id} className="rounded-xl border border-zinc-200 dark:border-[#1F1F2E] bg-zinc-50 dark:bg-[#09090B] p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">#{order.id.toString().slice(-6)}</p>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{order.customerName}</p>
                  </div>
                  <div className="relative inline-block w-[130px]">
                    <select
                      value={order.status}
                ...
                <div className="mt-3 text-xs text-zinc-500 dark:text-zinc-400 space-y-1">
                  <p>Items: <span className="text-zinc-700 dark:text-zinc-300">{order.items.length} product(s)</span></p>
                  <p className="truncate" title={order.items.map(i => i.name).join(', ')}>
                    Products: <span className="text-zinc-700 dark:text-zinc-300">{order.items.map(i => i.name).join(', ')}</span>
                  </p>
                </div>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => navigateTo('add-delivery', order)}
                      title="Create Delivery"
                      className="text-blue-500 hover:text-blue-700 dark:hover:text-blue-300 transition-colors inline-flex items-center justify-center p-2 rounded-lg border border-zinc-200 dark:border-[#272730]"
                    >
                      <Truck size={14} />
                    </button>
                    <button onClick={() => handleDelete(order.id)} className="text-red-500 hover:text-red-700 dark:hover:text-red-300 transition-colors inline-flex items-center justify-center p-2 rounded-lg border border-red-200 dark:border-red-500/30">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
