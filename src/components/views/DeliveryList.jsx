  ...
  return (
    <div className="animate-in fade-in duration-300 w-full">
      <div className="bg-white dark:bg-[#111116] border border-zinc-200 dark:border-[#1F1F2E] rounded-2xl overflow-hidden shadow-sm dark:shadow-lg w-full">
        ...
        {/* //ALLU: Keep desktop table view for larger screens and move small screens to a card layout. */}
        ...
        {/* //ALLU: Add mobile-friendly delivery cards to eliminate narrow-screen column misalignment. */}
        <div className="md:hidden p-4 space-y-3">
          {filteredDeliveries.length === 0 ? (
            ...
            filteredDeliveries.map((delivery, i) => (
              <div key={delivery.id} className="rounded-xl border border-zinc-200 dark:border-[#1F1F2E] bg-zinc-50 dark:bg-[#09090B] p-4">
                <div className="flex items-start justify-between gap-3">
                  <StatusDotLabel
                    ...
                    textClassName="text-zinc-900 dark:text-zinc-100"
                  />
                  <button onClick={() => handleDelete(delivery.id)} className="text-red-500 hover:text-red-700 dark:hover:text-red-300 transition-colors inline-flex items-center justify-center p-2 rounded-lg border border-red-200 dark:border-red-500/30">
                    <Trash2 size={14} />
                  </button>
                </div>

                <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                  <p>Size: <span className="text-zinc-700 dark:text-zinc-300">{delivery.size}</span></p>
                  <p>Amount: <span className="text-zinc-700 dark:text-zinc-300">{delivery.amount || '-'}</span></p>
                  <p className="col-span-2">Location: <span className="text-zinc-700 dark:text-zinc-300">{delivery.location}</span></p>
                </div>
