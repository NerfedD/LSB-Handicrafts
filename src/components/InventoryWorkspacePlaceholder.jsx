 ...
 *
 * Renders inside the shared layout/Shell like every other screen, so the
 * sidebar shows the real signed-in person and hides the admin-only items from
 * non-Admins on its own — see the NAV_TREE filter there. SECTION_OF in
 * utils/navigation already maps "workspace" onto the Dashboard nav item, so
 * that entry stays lit while this is open.
 */
      ...
      <Card className="mt-7">
        ...
        <div className="grid grid-cols-1 border-t border-[#17263a0f] sm:grid-cols-3">
          {MODULES.map((module, index) => {
            ...
            // arguments. Same dance as layout/Shell.
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-[#17263a0a]">
                    <Icon className="h-4 w-4 text-[#5f6875]" />
                  </span>
                ...
                <span className="mt-3 inline-flex items-center gap-2 rounded-full bg-[#9a610014] px-3 py-1 text-[11.5px] font-semibold text-[#8a5600]">
