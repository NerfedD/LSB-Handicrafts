...
- **Orders carry a `customer_id`** where the customer could be identified, so renaming somebody no longer loses their history and two people who share a name no longer share one. The link is set when the typed name matches exactly one customer record. The customer field on the order form stays free text on purpose — a walk-in can be served without being enrolled first — so an order naming nobody on file has no id and is still matched by name, as every order was before. Both threads are followed wherever orders are gathered (see `ordersByCustomer` in `src/utils/customers.js`).
